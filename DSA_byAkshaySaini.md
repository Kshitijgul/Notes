...
![alt text](blob:https://markdownviewer.pages.dev/ad11f5e0-6b9c-4ba9-a983-369ea559472b)
...
```mermaid
flowchart TD
    A[🤔 I have a problem] --> B{Need fast lookup?}
    B -->|Yes| C[🗂️ Hash Table / HashMap]
    B -->|No| D{Is order important?}
    D -->|Yes| E{Need LIFO?}
    D -->|No| F{Hierarchical data?}
    E -->|Yes| G[📚 Stack]
    E -->|No| H{Need FIFO?}
    H -->|Yes| I[🎟️ Queue]
    H -->|No| J[📦 Array / Linked List]
    F -->|Yes| K{Need to navigate paths?}
    F -->|No| L[📦 Array]
    K -->|Yes| M[🕸️ Graph]
    K -->|No| N[🌳 Tree]
```
